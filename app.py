...
def _craft_dashboard_block(df_detail: pd.DataFrame) -> None:
    ...
    st.altair_chart(
        base.mark_bar(size=50).encode(
            y=alt.Y("hours:Q", title="Hours"),
            color=alt.Color("Type:N", scale=color_scale),
        ),
        use_container_width=True,
    )
    # Prepare breakdown table showing hours and percent with a single decimal place.
    breakdown_df = agg[["Type", "hours", "percent"]].copy()
    # Rename columns for display: capitalise "hours" and use a percent sign
    breakdown_df = breakdown_df.rename(columns={"hours": "Hours", "percent": "%"})
    # Round numeric columns to one decimal place
    breakdown_df["Hours"] = breakdown_df["Hours"].astype(float).round(1)
    breakdown_df["%"] = breakdown_df["%"].astype(float).round(1)
    st.caption("Breakdown (Hours & % of Craft Total)")
    st.dataframe(_style_breakdown(breakdown_df), use_container_width=True, hide_index=True)
