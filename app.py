...
def _craft_dashboard_block(df_detail: pd.DataFrame) -> None:
    ...
    # Prepare breakdown table showing only hours (two decimal places).
    breakdown_df = agg[["Type", "hours"]].copy()
    # Rename columns for display: capitalise "hours".
    breakdown_df = breakdown_df.rename(columns={"hours": "Hours"})
    # Round Hours to two decimal places
    breakdown_df["Hours"] = breakdown_df["Hours"].astype(float).round(2)
