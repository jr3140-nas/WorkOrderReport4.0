...
def _create_detail_table_figures(df_detail: pd.DataFrame, craft_name: str, rows_per_page: int = 20) -> List[plt.Figure]:
    ...
    # Wrap text in the 'Problem' column only when it exceeds a threshold to
    # prevent horizontal overflow.  This helps keep the table within page
    # margins but avoids wrapping unnecessarily.  Row heights are scaled
    # uniformly (see below) to accommodate wrapped content.
    # Wrap text in the Problem and Description columns when it exceeds a
    # threshold.  This prevents very long strings from forcing the table
    # outside the page margins.  Long words (e.g., codes with underscores) are
    # allowed to break.
    def _wrap_if_needed(val: object, limit: int = 50) -> str:
    ...
    for col_name in ["Problem", "Description"]:
        if col_name in df_pdf.columns:
            df_pdf[col_name] = df_pdf[col_name].apply(_wrap_if_needed)
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        # Adjust column widths based on content, then scale uniformly so
        # the entire table stays within the page margins.  Shrinking the
        # horizontal scale factor slightly (e.g., 0.9) leaves a margin on
        # either side of the table.  Vertical scaling enlarges row heights
        # modestly to accommodate wrapped text.
        table.auto_set_column_width(col=list(range(len(col_labels))))
        # Shrink the table horizontally and expand vertically.  A stronger
        # horizontal scaling factor (e.g., 0.8) ensures the table remains
        # within the page even when there are many columns or long values.
        table.scale(0.8, 1.35)
        figures.append(fig)
    return figures
