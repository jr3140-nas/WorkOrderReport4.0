...
def _create_summary_figure(df_detail: pd.DataFrame, craft_name: str) -> plt.Figure:
    ...
    # Use a narrower bar width so the bar does not consume the full axis width,
    # especially when there is only a single category.  The margins call
    # below adds extra whitespace on either side of the bar(s).
    ax_bar.bar(agg["Type"].astype(str), agg["hours"], color=colors_list, width=0.6)
...
def _create_detail_table_figures(df_detail: pd.DataFrame, craft_name: str, rows_per_page: int = 20) -> List[plt.Figure]:
    """
    Split a detailed DataFrame into multiple figures for inclusion in the PDF.
    Each page shows up to ``rows_per_page`` rows.  Column widths are auto
    adjusted.  A page title is added to each figure indicating the craft
    ...
    # Only wrap long text in the 'Problem' column when necessary.  If the text
    # exceeds a threshold (e.g., 60 characters), it will be wrapped with
    # newlines.  Otherwise it is left unchanged.  This allows the table to
    # utilize the full page width for shorter entries.
    if "Problem" in df_pdf.columns:
        def _wrap_if_needed(val: object, limit: int = 60) -> str:
            s = str(val) if val is not None else ""
            return textwrap.fill(s, width=limit) if len(s) > limit else s
        df_pdf["Problem"] = df_pdf["Problem"].apply(_wrap_if_needed)
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        chunk = df_pdf.iloc[start : start + rows_per_page]
        fig = plt.figure(figsize=(11, 8.5))
        ...
        # Do not auto-adjust column widths here; keeping uniform widths across pages
        # helps maintain fixed table size within the page margins.

        # Adjust row heights individually when cells wrap.  Compute a base
        # height relative to the number of rows so the table fits in the
        # allocated axis height (0.85).  The header row is assigned one unit
        # of base height.  Each wrapped line adds another unit to its row.
        n_rows = len(chunk)
        base_height = 1.0 / (n_rows + 1)  # relative height per row
        # Set header row height
        for col_idx in range(len(col_labels)):
            cell = table._cells[(0, col_idx)]
            cell.set_height(base_height)
            cell.set_text_props(va="center")
        # Determine index of the Problem column
        problem_idx = col_labels.index("Problem") if "Problem" in col_labels else -1
        for row_offset, (_, row) in enumerate(chunk.iterrows(), start=1):
            if problem_idx >= 0:
                # count lines in the Problem cell
                lines = str(row["Problem"]).count("\n") + 1
            else:
                lines = 1
            row_height = base_height * lines
            for col_idx in range(len(col_labels)):
                cell = table._cells[(row_offset, col_idx)]
                cell.set_height(row_height)
                # Align text to the top for wrapped cells
...
def _craft_dashboard_block(df_detail: pd.DataFrame) -> None:
    ...
    # Add padding to the ordinal x-axis so single bars do not span the full width.
