...
def _create_detail_table_figures(df_detail: pd.DataFrame, craft_name: str, rows_per_page: int = 20) -> List[plt.Figure]:
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        table.auto_set_column_width(col=list(range(len(col_labels))))
        # Adjust row height if any cells wrap to multiple lines.  Determine the
        # maximum number of lines in the Problem column within this chunk and
        # scale the table accordingly.  Each additional line increases the
        # required row height by ~20%.
        if "Problem" in chunk.columns:
            max_lines = chunk["Problem"].apply(lambda s: str(s).count("\n") + 1).max()
            if max_lines > 1:
                # scale rows vertically; keep width scaling at 1
                table.scale(1, 1 + 0.2 * (max_lines - 1))
        figures.append(fig)
    return figures
