...
# === PDF Export & Email (added) ===
try:
    from export_pdf import build_dashboard_pdf
    from email_utils import send_email_with_attachment
except Exception as _e:
    build_dashboard_pdf = None

def _safe_selected_label():
    try:
...
if 'report' in globals() and report.get("full_detail") is not None and not report["full_detail"].empty and build_dashboard_pdf:
    pdf_bytes = build_dashboard_pdf(report["full_detail"], title_suffix=_safe_selected_label())
    st.download_button(
        "Download dashboard (PDF)",
        data=pdf_bytes,
        file_name=f"workorder_dashboard_{_safe_selected_label().replace('/', '-')}.pdf",
        mime="application/pdf",
        type="primary"
    )

    with st.expander("Email this PDF"):
        to_addr = st.text_input("Send to (email address)", key="pdf_email_to", placeholder="name@example.com")
        subject = st.text_input("Subject", value=f"Work Order Dashboard — {_safe_selected_label()}")
        body = st.text_area("Message", value="Attached is the latest Work Order Dashboard PDF.")
        colA, colB = st.columns([1,3])
        with colA:
            if st.button("Send email", type="secondary", disabled=not bool(to_addr and pdf_bytes)):
                try:
                    result = send_email_with_attachment(
                        ...
                        attachment_filename=f"workorder_dashboard_{_safe_selected_label().replace('/', '-')}.pdf",
                        # These can be provided via environment variables in your deployment
    ...
    st.info("Upload data to enable PDF export.")
