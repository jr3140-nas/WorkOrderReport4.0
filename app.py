...
def _type_color(val: str) -> str:
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return ""
    s = str(val).strip()
    return TYPE_COLOR_MAP.get(s, "")

def style_type_column(df: pd.DataFrame):
    # Use Styler.map for direct element-wise mapping
    styler = df.style.map(lambda v: f"background-color: {_type_color(v)}; color: #1f1f1f;" if _type_color(v) else "", subset=["Type"])
    return styler

def render_table(df: pd.DataFrame, height: int, column_config: dict):
    styled = style_type_column(df)
    try:
        st.dataframe(styled, use_container_width=True, hide_index=True, height=height, column_config=column_config)
    except Exception:
        # Fallback to raw HTML if the environment ignores Styler in st.dataframe
        html = styled.to_html()
...
col_cfg = {
    ...
    "Description": st.column_config.TextColumn("Description", width=420),
...
for craft_name, payload in report["groups"]:
    ...
    render_table(df_detail, _auto_height(df_detail), col_cfg)
    st.markdown("---")
