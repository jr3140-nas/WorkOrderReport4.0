...
# Force specific colors for certain types
_TYPE_COLOR_OVERRIDES = {
    "PM Inspection": "#009E73",  # green
    "Emergency Order": "#D55E00"  # red
}

# Update TYPE_COLOR_HEX with overrides
for _k, _v in _TYPE_COLOR_OVERRIDES.items():
    TYPE_COLOR_HEX[_k] = _v
...
# Plant standard: PM Inspection = green, Emergency Order = red
...
def _pie_for_group(df_detail: pd.DataFrame, title: str):
    ...
    # Larger canvas to reduce crowding; we will place labels outside with leader lines
    ...
    # Draw outside labels with extension (leader) lines
    import math
    label_fs = 8  # base font size
    for w, lbl, val in zip(wedges, labels, values):
        ...
        x = math.cos(ang_rad)
        y = math.sin(ang_rad)
        # End point for text (slightly outside the pie)
        r_text = 1.25
        tx = r_text * x
        ty = r_text * y
    ...
    # Reduce margins so the chart stays compact
    plt.tight_layout()
    return fig
