...
def _create_detail_table_figures(df_detail: pd.DataFrame, craft_name: str, rows_per_page: int = 20) -> List[plt.Figure]:
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        # Do not auto-adjust column widths.  Using uniform widths keeps the table
        # within the page margins regardless of cell content.
        # Apply a uniform vertical scale to ensure wrapped text fits; this makes
        # all rows slightly taller but prevents content from being cut off.
        table.scale(1, 1.25)
        figures.append(fig)
    return figures
