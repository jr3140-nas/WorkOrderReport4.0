...
def prepare_report_data(time_df: pd.DataFrame,
    ...
    # Sort detail rows and coerce types for clean display
    merged = merged.sort_values(["Craft Description", "Name", "Work Order Number"])
    # Work Order Number as string for nicer display (drop trailing .0)
    merged["Work Order Number"] = merged["Work Order Number"].astype(str).str.replace(r"\.0$", "", regex=True)
    # Sum of Hours as 2 decimals (still export unrounded values if preferred)
...
st.title("Work Order Reporting App")  # per instruction: keep this exact text
...
if report["unmapped_people"]:
    with st.expander("Unmapped personnel (not found in Address Book)", expanded=False):
        df_un = pd.DataFrame(report["unmapped_people"], columns=["AddressBookNumber", "Name"]).sort_values("Name")
        st.dataframe(df_un, use_container_width=True, hide_index=True, height=_auto_height(df_un))

# Consistent sizing for all displayed columns
col_cfg = {
    "Name": st.column_config.TextColumn("Name", width="medium"),
    "Work Order Number": st.column_config.TextColumn("Work Order Number", width="medium"),
    "Sum of Hours": st.column_config.NumberColumn("Sum of Hours", format="%.2f", width="medium"),
