...
# Sidebar controls for filtering detail tables.  These filters affect only
# the raw data tables displayed under each craft and do not modify the
# breakdown metrics or charts.  Users can choose to filter by Name or by
# Work Order #, and select one or more values accordingly.
st.sidebar.subheader("Table Filters")
filter_option = st.sidebar.radio(
    "Filter tables by", options=["None", "Name", "Work Order #"], index=0
)
selected_names = []  # type: List[str]
selected_wos: List[Any] = []
if filter_option == "Name":
    # Collect unique names across all crafts
    all_names = sorted(
        report.get("full_detail", pd.DataFrame()).get("Name", pd.Series(dtype=str)).dropna().unique().tolist()
    )
    selected_names = st.sidebar.multiselect("Select Name(s)", options=all_names)
elif filter_option == "Work Order #":
    # Collect unique work order numbers across all crafts
    all_wos = sorted(
        report.get("full_detail", pd.DataFrame()).get("Work Order #", pd.Series(dtype=str)).dropna().unique().tolist()
    )
    selected_wos = st.sidebar.multiselect("Select Work Order #(s)", options=all_wos)
...
for craft_name, payload in report["groups"]:
    ...
    # Apply table filtering based on the sidebar selections.  Only the tables are
    # filtered; metrics and charts remain based on the full detail.
    df_display = df_detail
    if filter_option == "Name" and selected_names:
        df_display = df_detail[df_detail["Name"].isin(selected_names)]
    elif filter_option == "Work Order #" and selected_wos:
        df_display = df_detail[df_detail["Work Order #"].isin(selected_wos)]
    st.dataframe(
