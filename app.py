...
# __PDF_CHARTS_INSERTED__: add bar chart and breakdown table for this craft
try:
    _df = payload["detail"].copy()
    _df["Sum of Hours"] = pd.to_numeric(_df["Sum of Hours"], errors="coerce").fillna(0.0)
    _agg = (_df.groupby("Type", dropna=False)["Sum of Hours"]
              .sum()
              .reset_index()
              .rename(columns={"Sum of Hours":"hours"})
              .sort_values("hours", ascending=False))
    _total = float(_agg["hours"].sum()) if not _agg.empty else 0.0
    _agg["percent"] = (_agg["hours"]/_total*100.0).where(_total>0, 0.0)
    # --- Create bar chart with matplotlib ---
    fig, ax = plt.subplots(figsize=(8.0, 3.0), dpi=150)
    ax.bar(_agg["Type"].astype(str), _agg["hours"])
    ax.set_xlabel("Work Order Type")
    ax.set_ylabel("Hours")
    ax.set_title(f"Hours by Type — {craft}")
    ...
    plt.close(fig)
    _buf.seek(0)
    img = RLImage(_buf, width=520, height=195)  # scale to fit A4 landscape with margins
    story.append(Spacer(1, 6))
    story.append(img)
    story.append(Spacer(1, 6))
    # --- Breakdown small table ---
    _bd_cols = ["Type", "hours", "percent"]
    _bd = [[Paragraph("<b>Type</b>", header_style), Paragraph("<b>Hours</b>", header_style), Paragraph("<b>% of Craft</b>", header_style)]]
    for _, r in _agg[_bd_cols].iterrows():
        _bd.append([Paragraph(str(r["Type"]), cell_style),
                    Paragraph(f"{float(r['hours']):.2f}", cell_style),
                    Paragraph(f"{float(r['percent']):.1f}%", cell_style)])
    bd_tbl = Table(_bd, repeatRows=1, hAlign="LEFT")
