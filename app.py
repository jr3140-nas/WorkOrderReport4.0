...
def _create_detail_table_figures(
    ...
    col_widths = [0.20, 0.12, 0.10, 0.15, 0.20, 0.23]
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        ax.axis("off")
        # Build the table using the wrapped data.  All cells are left‑aligned.
        table = ax.table(
        ...
        # Set fixed column widths.  Matplotlib's internal table representation
        # stores widths as fractions of the axis width.  By assigning these
        # values directly we prevent any column from expanding beyond its
        # allotted space, ensuring the entire table fits within the page.
        for idx, w in enumerate(col_widths):
            table._colWidths[idx] = w
        # Determine a base height for all rows.  The table spans roughly 90%
        # of the axis height.  We allocate space for the header row plus
        # ``rows_per_page`` data rows.  Each row gets an equal share of the
        # available vertical space.
        n_rows_rendered = max(len(chunk), rows_per_page)
        base_height = 0.85 / (rows_per_page + 1)
        # Assign header height
        table._rowHeights[0] = base_height
        # Assign heights for each data row.  Rows with wrapped text are
        # doubled in height; otherwise they use the base height.  We detect
        # wrapping by looking for newline characters introduced by the
        # textwrap.fill above.
        for i in range(len(chunk)):
            row_index = i + 1  # row 0 is the header
            # Inspect the Description and Problem cells for newlines
            has_wrap = False
            for col in ["Description", "Problem"]:
                try:
                ...
                if "\n" in cell_text:
                    has_wrap = True
                    break
            table._rowHeights[row_index] = base_height * (2 if has_wrap else 1)
        figures.append(fig)
    return figures
