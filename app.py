...
def _type_color(val: str) -> str:
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return ""
    s = str(val).strip()
    color = TYPE_COLOR_MAP.get(s)
    if not color:
        return ""
    return f"background-color: {color}; color: #1f1f1f;"

def style_type_column(df: pd.DataFrame):
    # Return a pandas Styler without explicit type annotations to avoid pandas attribute errors
    styler = df.style.apply(lambda col: col.map(_type_color), subset=["Type"])
    return styler
...
for craft_name, payload in report["groups"]:
    ...
    styler = style_type_column(df_detail)
    st.dataframe(
        styler,
