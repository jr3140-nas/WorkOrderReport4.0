...
st.caption("Upload the Time on Work Order export. Shows: Name, Work Order Number, Sum of Hours, Type, Description, Problem.")
...
if report["unmapped_people"]:
    with st.expander("⚠️ Unmapped personnel (not found in Address Book)", expanded=False):
        st.dataframe(
            pd.DataFrame(report["unmapped_people"], columns=["AddressBookNumber", "Name"]).sort_values("Name"),
...
for craft_name, payload in report["groups"]:
    ...
    if show_person_summary and not payload["person_summary"].empty:
        ...
        st.dataframe(
            payload["person_summary"],
            use_container_width=True,
            hide_index=True,
            column_config={
                "Sum of Hours": st.column_config.NumberColumn("Sum of Hours", format="%.2f", width="small"),
                "Work Orders": st.column_config.NumberColumn("Work Orders", width="small"),
            }
        )
    ...
    if show_detail and not payload["detail"].empty:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Work Order Number": st.column_config.NumberColumn("Work Order Number", width="small"),
                "Sum of Hours": st.column_config.NumberColumn("Sum of Hours", format="%.2f", width="small"),
            }
        )
    st.markdown("---")
