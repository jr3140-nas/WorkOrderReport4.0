...
def _type_color(name: str) -> str:
    return TYPE_COLOR_HEX.get(name, DEFAULT_SLICE)
...
def _pie_for_group(df_detail: pd.DataFrame, title: str):
    ...
    if s.empty or s.fillna(0).sum() <= 0:
        return None
    labels = [("Unspecified" if (pd.isna(k) or str(k).strip()=="") else str(k)) for k in s.index.tolist()]
...
for craft_name, payload in report["groups"]:
    ...
    # Pie chart of hours by Type for this craft
