...
def build_pdf(report: Dict[str, Any], date_label: str) -> bytes:
    ...
    with PdfPages(buffer) as pdf:
        ...
        # Write the title and subtitle on a blank figure.  Use plt.axis('off') to hide
        # the axes on the title page.  Calling plt.axis rather than fig.axis avoids
        # triggering an AttributeError on certain Matplotlib versions where Figure
        # objects do not implement an axis() method.
        ...
        if isinstance(full_detail, pd.DataFrame) and not full_detail.empty:
            df_full = full_detail.copy()
            df_full["Sum of Hours"] = pd.to_numeric(df_full["Sum of Hours"], errors="coerce").fillna(0.0)
            agg_full = (
                df_full.groupby("Type", dropna=False)["Sum of Hours"]
                .sum()
                .reset_index()
                .rename(columns={"Sum of Hours": "hours"})
                .sort_values("hours", ascending=False)
            )
            total = float(agg_full["hours"].sum())
            top_type = agg_full.iloc[0]["Type"] if not agg_full.empty else "-"
            top_hours = agg_full.iloc[0]["hours"] if not agg_full.empty else 0.0
            top_pct = (top_hours / total * 100.0) if total > 0 else 0.0
            fig_full, ax_full = plt.subplots(figsize=(11, 5))
            default_color = "#333333"
            colors_list = []
            for t in agg_full["Type"]:
                if isinstance(t, str):
                    colors_list.append(_TYPE_COLORS.get(str(t), default_color))
                else:
                    colors_list.append(default_color)
            ax_full.bar(agg_full["Type"].astype(str), agg_full["hours"], color=colors_list)
            ax_full.set_title("Overall Hours by Work Order Type")
            ax_full.set_xlabel("Type")
            ax_full.set_ylabel("Hours")
            ax_full.tick_params(axis="x", rotation=45, labelsize=8)
            fig_full.text(0.02, 0.92, f"Total Hours: {total:,.2f}", fontsize=10)
            fig_full.text(0.02, 0.89, f"Top Type: {top_type}", fontsize=10)
            fig_full.text(0.02, 0.86, f"% in Top Type: {top_pct:.1f}%", fontsize=10)
        ...
        for craft_name, payload in report.get("groups", []):
            ...
            top_hours = agg.iloc[0]["hours"] if not agg.empty else 0.0
            top_pct = (top_hours / total * 100.0) if total > 0 else 0.0
            ...
            for t in agg["Type"]:
                if isinstance(t, str):
                    ...
                    colors_list.append(default_color)
            ax.bar(agg["Type"].astype(str), agg["hours"], color=colors_list)
            ax.set_title(f"{craft_name} — Hours by Work Order Type")
            ax.set_xlabel("Type")
            ax.set_ylabel("Hours")
            ax.tick_params(axis="x", rotation=45, labelsize=8)
            fig.text(0.02, 0.92, f"Total Hours: {total:,.2f}", fontsize=10)
            fig.text(0.02, 0.89, f"Top Type: {top_type}", fontsize=10)
            fig.text(0.02, 0.86, f"% in Top Type: {top_pct:.1f}%", fontsize=10)
            plt.tight_layout(rect=[0, 0, 1, 0.8])
            pdf.savefig(fig)
            plt.close(fig)
    buffer.seek(0)
