...
# Display columns (with renamed "Work Order #")
...
def load_timeworkbook(file_like) -> pd.DataFrame:
    ...
    # Normalize work order into 'Work Order Number'
    ...
    # Create display column 'Work Order #' as text (trim trailing .0)
    df["Work Order #"] = df["Work Order Number"].astype(str).str.replace(r"\.0$", "", regex=True)
...
st.title("Work Order Reporting App")  # keep exact title
...
# Column config: shrink 'Work Order #' and 'Sum of Hours'
col_cfg = {
    "Name": st.column_config.TextColumn("Name", width="medium"),
    "Work Order #": st.column_config.TextColumn("Work Order #", width="small"),
    "Sum of Hours": st.column_config.NumberColumn("Sum of Hours", format="%.2f", width="small"),
