...
# The traditional PDF export functionality has been disabled in favour of
# printing the dashboard directly from the browser.  This button hides the
# sidebar and triggers the browser's print dialog so that users can save the
# dashboard as a PDF (landscape) with scaling to fit the page.  After
# printing, the sidebar is restored.  The implementation uses a small
# HTML/JavaScript snippet embedded via streamlit.components.
import streamlit.components.v1 as components  # type: ignore
print_button_html = """
    <button id=\"dash-print-btn\" style=\"margin-top: 10px; padding: 6px 12px; font-size: 14px;\">
      Print Dashboard to PDF
    </button>
    <script>
    const btn = window.parent.document.getElementById('dash-print-btn');
    if (btn) {
      btn.addEventListener('click', () => {
        // Locate the sidebar element and hide it
        const sidebar = window.parent.document.querySelector('section[data-testid="stSidebar"]');
        if (sidebar) {
          sidebar.style.display = 'none';
        }
        // Give the layout time to adjust before printing
        setTimeout(() => {
          window.print();
