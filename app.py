...
def _create_detail_table_figures(df_detail: pd.DataFrame, craft_name: str, rows_per_page: int = 20) -> List[plt.Figure]:
    ...
    # Wrap long text in the 'Problem' column so that it doesn't run off the page.
    if "Problem" in df_pdf.columns:
        df_pdf["Problem"] = df_pdf["Problem"].astype(str).apply(lambda x: textwrap.fill(x, width=40))
