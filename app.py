...
def prepare_report_data(time_df: pd.DataFrame, addr_df: pd.DataFrame, craft_df: pd.DataFrame, target_date) -> dict:
    """Builds the structure:
       {
         "groups": [(craft_name, {"detail": df_for_craft}), ...],
         "full_detail": concatenated_df,
         "unmapped_people": [names with no craft]
       }
    """
    # Ensure Production Date is date-like and filter
    df = time_df.copy()
    if "Production Date" in df.columns:
        df["Production Date"] = pd.to_datetime(df["Production Date"], errors="coerce").dt.date
        df = df[df["Production Date"] == target_date]
    # Normalize columns expected by the UI
    for need in DISPLAY_COLUMNS:
        if need not in df.columns:
            df[need] = pd.NA
    # Join craft info
    if not addr_df.empty:
        key = "AddressBookNumber" if "AddressBookNumber" in df.columns and "AddressBookNumber" in addr_df.columns else "Name"
        join = df.join(addr_df.set_index(key), on=key, how="left", rsuffix="_addr")
        craft_col = "Craft Description"
        df["Craft Description"] = join[craft_col] if craft_col in join.columns else pd.NA
    else:
        df["Craft Description"] = pd.NA

    # Determine craft order
    if not craft_df.empty and "Craft Description" in craft_df.columns:
        craft_order = [str(x) for x in craft_df["Craft Description"].dropna().tolist()]
    else:
        craft_order = sorted([c for c in df["Craft Description"].dropna().unique().tolist()])
    if not craft_order:
        craft_order = ["Unassigned"]

    # Build detail rows
    detail = df[DISPLAY_COLUMNS + ["Craft Description"]].copy()
    detail["Sum of Hours"] = pd.to_numeric(detail["Sum of Hours"], errors="coerce").fillna(0.0)

    groups_payload = []
    for craft in craft_order:
        g_detail = detail[detail["Craft Description"].fillna("Unassigned") == craft][DISPLAY_COLUMNS].copy()
        if g_detail.empty:
            continue
        groups_payload.append((str(craft), {"detail": g_detail}))

    full_detail = detail[DISPLAY_COLUMNS].copy()

    # Unmapped people
    unmapped = df.loc[df["Craft Description"].isna(), "Name"].dropna().unique().tolist()
