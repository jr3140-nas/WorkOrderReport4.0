...
def build_pdf(report: Dict[str, Any], date_label: str) -> bytes:
    ...
    for craft_name, payload in report["groups"]:
        ...
        try:
            ...
            plt.bar(agg_pdf["Type"], agg_pdf["hours"])
            plt.title("Hours by Work Order Type")
            plt.ylabel("Hours")
            ...
            plt.close(fig)
            buf.seek(0)
            story.append(RLImage(buf, width=500, height=200))
            ...
            bd_tbl.setStyle(TableStyle([
                ...
                ("ALIGN", (0,1), (0,-1), "RIGHT"),   # Type right
                ("ALIGN", (1,1), (1,-1), "LEFT"),    # Hours left
