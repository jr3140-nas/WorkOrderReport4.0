...
# === Mini-dashboard helpers (inserted) ===
...
_TYPE_COLORS = {
    "Break In": "#d62728",                 # red
    "Maintenance Order": "#1f77b4",        # blue
    "Urgent Corrective": "#ff7f0e",        # orange
    "Emergency Order": "#d62728",          # red
    "PM Restore/Replace": "#2ca02c",       # green
    "PM Inspection": "#2ca02c",            # green
    "Follow Up Maintenance Order": "#d6d61c", # yellow-ish
    "Project": "#9467bd"                   # purple
}
...
def _craft_dashboard_block(df_detail: pd.DataFrame):
    ...
    if df_detail.empty:
        return
    agg = (df_detail.groupby("Type", dropna=False)["Sum of Hours"]
                    .sum()
                    .reset_index()
    ...
    # Altair charts with fixed palette
    base = alt.Chart(agg).encode(
        x=alt.X("Type:N", sort="-y", title="Work Order Type"),
        tooltip=[alt.Tooltip("Type:N"), alt.Tooltip("hours:Q", format=".2f"), alt.Tooltip("percent:Q", format=".1f")]
    )
    ...
    st.altair_chart(
        base.mark_bar().encode(y=alt.Y("percent:Q", title="% of Craft Hours", axis=alt.Axis(format="~s")),
                               color=alt.Color("Type:N", scale=color_scale)),
        use_container_width=True
    )
    ...
    st.dataframe(agg.assign(percent=lambda d: d["percent"].round(1))
                    .rename(columns={"percent":"% of Craft"}),
