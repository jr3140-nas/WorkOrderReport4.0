...
def _create_summary_figure(df_detail: pd.DataFrame, craft_name: str) -> plt.Figure:
    ...
    # Bar chart axis
    ax_bar = fig.add_axes([0.05, 0.35, 0.55, 0.5])  # [left, bottom, width, height]
    ...
    # Metrics text block
    fig.text(0.05, 0.28, f"Total Hours: {total:,.2f}", fontsize=10)
    fig.text(0.05, 0.25, f"Top Type: {top_type}", fontsize=10)
    fig.text(0.05, 0.22, f"% in Top Type: {top_pct:.1f}%", fontsize=10)

    # Breakdown table axis
    ax_tbl = fig.add_axes([0.65, 0.35, 0.3, 0.5])
    ax_tbl.axis("off")
    # Prepare cell content for table: convert hours and percent to strings
    tbl_data = [
...
def build_pdf(report: Dict[str, Any], date_label: str) -> bytes:
    ...
    with PdfPages(buffer) as pdf:
        ...
        if isinstance(full_detail, pd.DataFrame) and not full_detail.empty:
            ...
            # Detail pages for overall data (optional): include the entire filtered detail
            # Splitting into multiple pages ensures large datasets are presented cleanly.
            detail_figs = _create_detail_table_figures(full_detail, "Overall Detail")
